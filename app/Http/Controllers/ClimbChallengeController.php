...
class ClimbChallengeController extends Controller
{
    ...
    /**
     * Get dashboard data as JSON for auto-updates
     */
    ...
    public function checkForUpdates(Request $request)
    {
        ...
        // Check if there are any updates in summoner_tracks, league_matches, or summoners since the last update
        $hasUpdates = DB::table('summoner_tracks')
            ->where('updated_at', '>', $lastUpdateTime)
            ->exists();

        if (!$hasUpdates) {
            $hasUpdates = DB::table('league_matches')
                ->where('updated_at', '>', $lastUpdateTime)
                ->exists();
        }

        if (!$hasUpdates) {
            $hasUpdates = DB::table('summoners')
                ->where('updated_at', '>', $lastUpdateTime)
                ->exists();
        }
